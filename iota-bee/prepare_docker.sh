...
set_config "${configPath}" ".alias"                       "\"${BEE_NODE_ALIAS:-bee}\""
...
set_config "${configPath}" ".dashboard.auth.passwordHash" "\"${DASHBOARD_PASSWORD}\"" "secret"
set_config "${configPath}" ".dashboard.auth.passwordSalt" "\"${DASHBOARD_SALT}\"" "secret"
