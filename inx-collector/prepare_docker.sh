...
# Validate INX_COLLECTOR_LEDGER_NETWORK config
...
set_config "${configPath}" ".storage.defaultBucketExpirationDays" "\"${INX_COLLECTOR_STORAGE_BUCKET_EXPIRATION_DAYS:-30}\""
set_config "${configPath}" ".POI.hostUrl"                         "\"${INX_COLLECTOR_POI_URL:-http://inx-poi:9687}\""
...
if [[ -z "${INX_COLLECTOR_STORAGE_BUCKET_NAME}" ]]; then
  set_config "${configPath}" ".storage.defaultBucketName"         "\"${INX_COLLECTOR_LEDGER_NETWORK}-${INX_COLLECTOR_NETWORK:-mainnet}-default\""
else
  set_config "${configPath}" ".storage.defaultBucketName"         "\"${INX_COLLECTOR_STORAGE_BUCKET_NAME}\""
fi
