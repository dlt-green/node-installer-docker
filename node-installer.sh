...
VAR_SHIMMER_CORE_VERSION='1.0.0-beta.3'
VAR_SHIMMER_CORE_UPDATE=0
...
VAR_SHIMMER_INX_INDEXER_VERSION='2.0.0-beta.1'
VAR_SHIMMER_INX_MQTT_VERSION='2.0.0-beta.1'
VAR_SHIMMER_INX_BLOCKISSUER_VERSION='1.0.0-beta.1'
VAR_SHIMMER_INX_VALIDATOR_VERSION='1.0.0-beta.1'
VAR_SHIMMER_INX_DASHBOARD_VERSION='2.0.0-beta.3'
...
Dashboard() {
	...
	5) VAR_NETWORK=2; VAR_NODE=5; VAR_DIR='shimmer-core'
	   if [ "$opt_mode" ]; then ShimmerCore; clear; exit; else SubMenuMaintenance; fi ;;
...
SubMenuMaintenance() {
	...
	1) if [ "$VAR_NETWORK" = 1 ] && [ "$VAR_NODE" = 1 ]; then IotaHornet; fi
	   ...
	   if [ "$VAR_NETWORK" = 2 ] && [ "$VAR_NODE" = 5 ]; then ShimmerCore; fi
...
#		fi
		...
		echo "IOTA_CORE_AUTOPEERING_BOOTSTRAP_PEER=\"/dns/access-0.h.nova-testnet.iotaledger.net/tcp/15600/p2p/12D3KooWRKnwe6FrswrVSq2jFuDTBEAb7iAKUTLkmCK6MDPBySMo\"" >> .env
