...
Dashboard() {
	...
	elif [ -d /var/lib/shimmer-plugins/inx-chronicle ]; then ix=$rd; fi

	if [ "$(docker container inspect -f '{{.State.Status}}' shimmer-plugins'.inx-groupfi' 2>/dev/null)" = 'running' ]; then
	  if [ ix != "$rd" ]; then ix=$gn; fi
	elif [ -d /var/lib/shimmer-plugins/inx-groupfi ]; then ix=$rd; fi
	...
	if [ "$VAR_NETWORK" = 2 ] && [ "$VAR_NODE" = 22 ]; then
		if [ -f /var/lib/$VAR_DIR/.env ]; then
			if [ $(cat .env 2>/dev/null | grep INX_GROUPFI_VERSION | cut -d '=' -f 2) = $VAR_SHIMMER_INX_GROUPFI_VERSION ]; then
				...
				echo "$ca""Network/Plugin: "$(echo $VAR_DIR | sed 's/\-plugins//')" | installed: v."$(cat .env 2>/dev/null | grep INX_GROUPFI_VERSION | cut -d '=' -f 2)" | available: v.$VAR_SHIMMER_INX_GROUPFI_VERSION""$xx"
			fi
		else
			echo "$ca""Network/Plugin: "$(echo $VAR_DIR | sed 's/\-plugins//')" | available: v.$VAR_SHIMMER_INX_GROUPFI_VERSION""$xx"
		fi
	fi
...
SubMenuPlugins() {
	...
	echo "║""$ca""$VAR_DOMAIN""$xx""║"
	echo "║                                                                             ║"
	echo "║                              1. ""$ixc""INX-Chronicle (IOTA)""$xx""                        ║"
	echo "║                              2. ""$ixg""INX-GroupFi (TanglePay)""$xx""                     ║"	
	echo "║                              X. Management Dashboard                        ║"
	echo "║                                                                             ║"
	echo "╚═════════════════════════════════════════════════════════════════════════════╝"
	echo ""
...
ShimmerChronicle() {
	...
	if [ "$VAR_CONF_RESET" = 1 ]; then
		...
		read -r -p '> ' VAR_TMP
		if [ -n "$VAR_TMP" ]; then VAR_SHIMMER_INX_CHRONICLE_MONGODB_USERNAME=$VAR_TMP; elif [ -z "$MONGODB_USERNAME" ]; then VAR_SHIMMER_INX_CHRONICLE_MONGODB_USERNAME=$VAR_DEFAULT; fi
		...
		read -r -p '> ' VAR_TMP
		if [ -n "$VAR_TMP" ]; then VAR_SHIMMER_INX_CHRONICLE_INFLUXDB_USERNAME=$VAR_TMP; elif [ -z "$INFLUXDB_USERNAME" ]; then VAR_SHIMMER_INX_CHRONICLE_INFLUXDB_USERNAME=$VAR_DEFAULT; fi
	...
	echo ""
	echo "╔═════════════════════════════════════════════════════════════════════════════╗"
	echo "║                         Start Shimmmer-INX-Chronicle                        ║"
	echo "╚═════════════════════════════════════════════════════════════════════════════╝"
	echo ""
...
ShimmerGroupFi() {
	...
	echo ""
	echo "Download Package... install.tar.gz"
	wget -cO - "$ShimmerGroupfiPackage" -q > install.tar.gz

	if [ "$(shasum -a 256 './install.tar.gz' | cut -d ' ' -f 1)" = "$ShimmerGroupfiHash" ]; then
		echo "$gn"; echo 'Checking Hash of Package successful...'; echo "$xx"
	else
		echo "$rd"; echo 'Checking Hash of Package failed...'
		echo 'Package has been tampered, Installation aborted for your Security!'
	...
	VAR_SHIMMER_INX_GROUPFI_LEDGER_NETWORK='shimmer'
	...
	if [ "$VAR_CONF_RESET" = 1 ]; then
		...
		echo "INX_GROUPFI_NETWORK=$VAR_SHIMMER_HORNET_NETWORK" >> .env
	...
	echo ""
	echo "╔═════════════════════════════════════════════════════════════════════════════╗"
	echo "║                         Start shimmer-inx-groupfi                           ║"
	echo "╚═════════════════════════════════════════════════════════════════════════════╝"
	echo ""
