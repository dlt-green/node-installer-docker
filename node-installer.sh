...
VAR_SHIMMER_WASP_VERSION='0.5.0-alpha.2'
VAR_SHIMMER_WASP_CLI_VERSION='0.5.0-alpha.2'
...
Pipe() {
	...
	if [ $VAR_CONF_RESET = 1 ]; then
		...
		if [ -n "$VAR_SEED" ]; then
		  ...
		  VAR_SEED=$(cat $credentials 2>/dev/null | grep Seed | cut -d ':' -f 2)
		  VAR_ADDRESS=$(cat $credentials 2>/dev/null | grep Address | cut -d ':' -f 2)
		fi
